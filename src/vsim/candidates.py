...
def generate_default_candidates(
    n_candidates: int, electorate: np.ndarray
) -> np.ndarray:
    """
...
def get_swedish_parties() -> np.ndarray:
...
CANDIDATE_OPTIONS: Dict[str, Callable] = {
    ...
    "swedish-parties": partial(get_swedish_parties),
}


def setup_candidates(
    candidates: int, issues: int, scenario: str, seed: Optional[int] = None
) -> np.ndarray:
    return CANDIDATE_OPTIONS[scenario](candidates, issues, seed=seed)
